...
namespace Inventory_Management_System.BusinessLogic.Services.Implementation
{
    public class DeliveryManService: IDeliveryManService
    {
        ...
        {
            var deliveryMan = await _unitOfWork.DeliveryMen.GetByIdAsync(d => d.DeliveryManID == id, d => d.Manager, d => d.Shipments);
            if (deliveryMan == null)
            {
                return null;
            }
        ...
        {
            var IfExists = await _unitOfWork.DeliveryMen.GetByIdAsync(d => d.FullName == deliveryManDto.FullName);
            if (IfExists != null)
            {
        ...
        {
            var existingDeliveryMan = await _unitOfWork.DeliveryMen.GetByIdAsync(d => d.DeliveryManID == id);
        ...
        {
            var deliveryMan = await _unitOfWork.DeliveryMen.GetByIdAsync(d => d.DeliveryManID == id);
